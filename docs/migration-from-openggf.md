...
| The extraction preflight runs every reviewed native capture contract | Retained as ten disjoint prefix/exact selectors in `tracechaser_build.native_test_inventory`. Each selector locks its expected count and exact ordered-name digest; machine-readable results reject failures, skips, missing/extra names, and overlap. The immutable OpenGGF boundary reproduces as 153 distinct tests from 156 invocations (not the earlier mistaken 150 claim); the standalone boundary adds the external-fixture-root and producer-path-alias guards for exactly 155 distinct tests and 0 skips. |
