...
{
    /// <summary>
    /// Strict-v5 S3KTraceMetadataWriter assertions. Each profile's current
    /// expected metadata text uses a fixed recording date and requires the
    /// recorder fields, key order, indentation, hex widths,
    /// aux_schema_extras ordering, capture mode, and notes values. Removed
    /// version fields are absent; expected strings must not be relaxed to
    /// accommodate historical fixture shapes.
    /// </summary>
    internal static class S3KTraceMetadataWriterTests
    {
        ...
        /// <summary>
        /// Current MGZ output must NOT contain pre_trace_osc_frames. MGZ
        /// arms outside CNZ
