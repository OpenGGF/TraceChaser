...
for file in gpgx.wbx gpgx.wbx.zst source-bundle.tar source-bundle.tar.zst source-bundle.paths \
  ...
  [[ -f "$build/$file" && ! -L "$build/$file" ]] || fail "build output missing $file"
done
...
verify_stock "$stock" || fail "caller stock distribution differs before install"
caller_stock=$stock
stage=
cleanup() {
...
notices_tree=$(
  cd "$build/llvm-debian-notices"
  /usr/bin/find . \( -type f -o -type l \) -printf '%P\n' | LC_ALL=C /usr/bin/sort | while IFS= read -r path; do
    if [[ -L "$path" ]]; then
      /usr/bin/printf 'l\t%s\t%s\n' "$(/usr/bin/readlink "$path")" "$path"
    else
...
verify_stock "$stock" || fail "caller stock distribution changed during install"
