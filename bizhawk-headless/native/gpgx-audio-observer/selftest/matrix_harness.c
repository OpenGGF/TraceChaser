...
#ifndef TRACE_DEFERRED_ORIGIN
#define TRACE_DEFERRED_ORIGIN(reservation) ((reservation).blocker)
#endif
#ifndef TRACE_DEFERRED_CURRENT_OWNER
#define TRACE_DEFERRED_CURRENT_OWNER(reservation) ((reservation).blocker)
#endif

#ifndef ACTION_RESERVE_DEFERRED_BEGIN
#define ACTION_RESERVE_DEFERRED_BEGIN 11u
#endif
#ifndef ACTION_CONSUME_DEFERRED_BEGIN
...
static void observation_marker_alternatives(void)
{
  ...
  for(int abi=2;abi<=3;abi++)
  {
    assert(gpgx_audio_trace_disable()==TRACE_OK);
    local_config.abi_version=(uint16_t)abi;
    assert(gpgx_audio_trace_configure(&local_config,local_mask,local_kinds,
      local_hooks,&local_range)==TRACE_OK);
    assert(gpgx_audio_trace_begin_frame()==TRACE_OK);
    selftest_m68k_a7=0x89abcdefu;
    gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x200);
    assert(trace_event_count_value==1 && trace_events[0].payload_length==0
      && !memcmp(trace_events[0].payload,"\0\0\0\0\0\0\0\0",8));
    assert(gpgx_audio_trace_abort_frame()==TRACE_OK);
  }

  assert(gpgx_audio_trace_disable()==TRACE_OK);
  local_config.abi_version=4;
  assert(gpgx_audio_trace_configure(&local_config,local_mask,local_kinds,
    local_hooks,&local_range)==TRACE_OK);
  assert(gpgx_audio_trace_begin_frame()==TRACE_OK);
  selftest_m68k_a7=0x89abcdefu;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x200);
  assert(trace_event_count_value==1 && trace_events[0].payload_length==4
    && trace_events[0].payload[0]==0xef && trace_events[0].payload[1]==0xcd
    && trace_events[0].payload[2]==0xab && trace_events[0].payload[3]==0x89
    && !memcmp(&trace_events[0].payload[4],"\0\0\0\0",4));
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);
}
...
static void deferred_reserve_consume_matrix(void)
{
  ...
  c.watch_mask_bytes=8192; c.kind_count=6; c.hook_count=13; c.range_count=1;
  c.snapshot_bytes_total=9; c.event_capacity=65536; c.max_service_tokens_per_frame=65535;
  for (uint32_t i=0;i<6;i++)
  {
    k[i].kind_id=(uint8_t)(i+1); k[i].cancellation_range_count=1;
  }
  k[1].flags=KIND_TYPED_ASYNC|KIND_ALLOW_CONTINUATION|KIND_ALLOW_CHILDREN;
  k[1].continuation_frame_limit=2;
  k[2].flags=KIND_TYPED_ASYNC|KIND_ALLOW_CONTINUATION|KIND_ALLOW_CHILDREN;
  k[2].continuation_frame_limit=2;
  k[3].flags=KIND_ALLOW_CHILDREN;
  k[5].flags=KIND_TYPED_ASYNC|KIND_ALLOW_CONTINUATION;
  k[5].continuation_frame_limit=2;
  r.range_id=1; r.start=0x100; r.length=1;
  h[0].hook_token=1; h[0].action=ACTION_PUSH_BEGIN;
  h[0].cpu=GPGX_AUDIO_TRACE_CPU_Z80; h[0].pc=0x3a; h[0].service_kind=6;
  h[0].opcode_length=2; h[0].opcode[0]=0xd6; h[0].opcode[1]=0x81;
  ...
  h[2]=h[1]; h[2].hook_token=3; h[2].pc=0xc1; h[2].service_kind=3;
  h[3].hook_token=4; h[3].action=ACTION_PUSH_BEGIN;
  h[3].cpu=GPGX_AUDIO_TRACE_CPU_Z80; h[3].pc=0xc2;
  h[3].service_kind=6; h[3].expected_active_kind=4; h[3].opcode_length=1;
  h[3].opcode[0]=0xa2;
  h[4].hook_token=5; h[4].action=ACTION_RESERVE_DEFERRED_BEGIN;
  h[4].cpu=GPGX_AUDIO_TRACE_CPU_M68K; h[4].pc=0x71b4c;
  h[4].service_kind=4; h[4].expected_active_kind=6;
  h[4].opcode_length=8; h[4].opcode[0]=0x33; h[4].opcode[1]=0xfc;
  h[4].opcode[2]=0x01; h[4].opcode[3]=0x00; h[4].opcode[4]=0x00;
  h[4].opcode[5]=0xa1; h[4].opcode[6]=0x11; h[4].opcode[7]=0x00;
  h[5].hook_token=6; h[5].action=ACTION_OBSERVATION_MARKER;
  h[5].cpu=GPGX_AUDIO_TRACE_CPU_M68K; h[5].pc=0x71b82;
  h[5].expected_active_kind=2; h[5].opcode_length=6;
  h[5].opcode[0]=0x4d; h[5].opcode[1]=0xf9;
  h[5].opcode[2]=0x00; h[5].opcode[3]=0xff; h[5].opcode[4]=0xf0; h[5].opcode[5]=0x00;
  h[6]=h[5]; h[6].hook_token=7; h[6].action=ACTION_CONSUME_DEFERRED_BEGIN;
  h[6].service_kind=4;
  h[7]=h[5]; h[7].hook_token=8; h[7].expected_active_kind=3;
  h[8]=h[6]; h[8].hook_token=9; h[8].expected_active_kind=3;
  h[9]=h[6]; h[9].hook_token=10; h[9].expected_active_kind=6;
  h[10]=h[5]; h[10].hook_token=13; h[10].expected_active_kind=6;
  h[11].hook_token=11; h[11].action=ACTION_OBSERVATION_MARKER;
  h[11].cpu=GPGX_AUDIO_TRACE_CPU_M68K; h[11].pc=0x71bb2;
  h[11].expected_active_kind=4; h[11].opcode_length=4;
  h[11].opcode[0]=0x4a; h[11].opcode[1]=0x6e; h[11].opcode[2]=0x00; h[11].opcode[3]=0x0a;
  h[12].hook_token=12; h[12].action=ACTION_POP_END_AT_PC;
  h[12].cpu=GPGX_AUDIO_TRACE_CPU_M68K; h[12].pc=0x71c4c;
  h[12].expected_active_kind=4; h[12].range_count=1;
  h[12].opcode_length=2; h[12].opcode[0]=0x4e; h[12].opcode[1]=0x75;
  mask[0x3a>>3]=(uint8_t)(1u<<(0x3a&7));
  mask[0x77>>3]=(uint8_t)(1u<<(0x77&7));
  mask[0xc1>>3]|=(uint8_t)(1u<<(0xc1&7));
  mask[0xc2>>3]|=(uint8_t)(1u<<(0xc2&7));
  zram[0x3a]=0xd6; zram[0x3b]=0x81; zram[0x77]=0x1a;
  zram[0xc1]=0x1a; zram[0xc2]=0xa2;
  for (uint32_t i=0;i<c.hook_count;i++)
    if (h[i].cpu==GPGX_AUDIO_TRACE_CPU_M68K)
      for (uint32_t j=0;j<h[i].opcode_length;j++)
        rom[((h[i].pc+j)&0xffffu)^1u]=h[i].opcode[j];
  memset(&m68k,0,sizeof(m68k)); m68k.memory_map[0].base=rom; m68k.memory_map[7].base=rom;
  base_c=c; memcpy(base_k,k,sizeof(k)); memcpy(base_h,h,sizeof(h));
  memcpy(base_mask,mask,sizeof(mask));
#define RESTORE_DEFERRED_PAIR() do { c=base_c; memcpy(k,base_k,sizeof(k)); \
  memcpy(h,base_h,sizeof(h)); memcpy(mask,base_mask,sizeof(mask)); } while(0)
#define REJECT_DEFERRED_PAIR(change) do { RESTORE_DEFERRED_PAIR(); change; \
  assert(gpgx_audio_trace_disable()==TRACE_OK); \
  assert(gpgx_audio_trace_configure(&c,mask,k,h,&r)==TRACE_ABI_OR_CONFIG_LIMIT); } while(0)
  REJECT_DEFERRED_PAIR(c.abi_version=2);
  REJECT_DEFERRED_PAIR(h[3].expected_active_kind=2);
  REJECT_DEFERRED_PAIR(memmove(&h[3],&h[4],9*sizeof(h[0])); c.hook_count=12;
    mask[0xc2>>3]&=(uint8_t)~(1u<<(0xc2&7)));
  REJECT_DEFERRED_PAIR(memmove(&h[5],&h[4],9*sizeof(h[0])); h[4]=h[3];
    h[4].hook_token=14; h[4].pc=0xc3; c.hook_count=14;
    mask[0xc3>>3]|=(uint8_t)(1u<<(0xc3&7)));
  REJECT_DEFERRED_PAIR(memmove(&h[4],&h[5],7*sizeof(h[0])); c.hook_count=11);
  REJECT_DEFERRED_PAIR(memmove(&h[6],&h[5],7*sizeof(h[0])); h[5]=h[4];
    h[5].hook_token=13; c.hook_count=13);
  REJECT_DEFERRED_PAIR(memmove(&h[9],&h[10],2*sizeof(h[0])); c.hook_count=11);
  REJECT_DEFERRED_PAIR(memmove(&h[11],&h[10],2*sizeof(h[0])); h[10]=h[9];
    h[10].hook_token=13; c.hook_count=13);
  REJECT_DEFERRED_PAIR(memmove(&h[6],&h[7],5*sizeof(h[0])); c.hook_count=11);
  REJECT_DEFERRED_PAIR(memmove(&h[11],&h[10],2*sizeof(h[0])); h[10]=h[9];
    h[10].hook_token=13; h[10].expected_active_kind=5; c.hook_count=13);
  REJECT_DEFERRED_PAIR(memmove(&h[5],&h[6],6*sizeof(h[0])); c.hook_count=11);
  REJECT_DEFERRED_PAIR(memmove(&h[11],&h[10],2*sizeof(h[0])); h[10]=h[5];
    h[10].hook_token=13; c.hook_count=13);
  REJECT_DEFERRED_PAIR(h[5].pc=0x71b80);
  REJECT_DEFERRED_PAIR(h[5].opcode[0]=0x4e);
  REJECT_DEFERRED_PAIR(swap=h[5]; memmove(&h[4],&h[3],2*sizeof(h[0])); h[3]=swap;
    h[3].cpu=GPGX_AUDIO_TRACE_CPU_Z80; h[3].pc=0xc2;
    h[3].opcode_length=1; memset(h[3].opcode,0,sizeof(h[3].opcode)); h[3].opcode[0]=0xa2;
    mask[0xc2>>3]|=(uint8_t)(1u<<(0xc2&7)));
  REJECT_DEFERRED_PAIR(h[5].expected_active_kind=5);
  REJECT_DEFERRED_PAIR(h[5].service_kind=4);
  REJECT_DEFERRED_PAIR(h[6].service_kind=5);
  REJECT_DEFERRED_PAIR(memmove(&h[11],&h[10],2*sizeof(h[0])); h[10]=h[5];
    h[10].hook_token=13; h[10].expected_active_kind=6; c.hook_count=13);
  REJECT_DEFERRED_PAIR(memmove(&h[1],&h[2],10*sizeof(h[0])); c.hook_count=11;
    c.snapshot_bytes_total=8;
    mask[0x77>>3]&=(uint8_t)~(1u<<(0x77&7)));
  REJECT_DEFERRED_PAIR(memmove(&h[3],&h[2],10*sizeof(h[0])); h[2]=h[1];
    h[2].hook_token=13; h[2].pc=0x78; c.hook_count=13; c.snapshot_bytes_total=10;
    mask[0x78>>3]|=(uint8_t)(1u<<(0x78&7)));
  REJECT_DEFERRED_PAIR(h[1].expected_active_kind=5);
  REJECT_DEFERRED_PAIR(k[1].flags=0);
  REJECT_DEFERRED_PAIR(k[5].flags|=KIND_ALLOW_CHILDREN);
  REJECT_DEFERRED_PAIR(h[1].service_kind=6);
  REJECT_DEFERRED_PAIR(h[1].service_kind=4);
  REJECT_DEFERRED_PAIR(memmove(&h[12],&h[10],2*sizeof(h[0])); h[10]=h[5];
    h[10].hook_token=13; h[10].expected_active_kind=5; h[11]=h[6];
    h[11].hook_token=14; h[11].expected_active_kind=5; c.hook_count=14);
  REJECT_DEFERRED_PAIR(memmove(&h[3],&h[2],10*sizeof(h[0])); h[2]=h[1];
    h[2].hook_token=13; h[2].pc=0x78; h[2].expected_active_kind=2;
    h[2].service_kind=5; k[4].flags=KIND_ALLOW_CHILDREN; c.hook_count=13;
    c.snapshot_bytes_total=10;
    mask[0x78>>3]|=(uint8_t)(1u<<(0x78&7)));
  REJECT_DEFERRED_PAIR(h[4].service_kind=0);
  REJECT_DEFERRED_PAIR(h[4].service_kind=7);
  REJECT_DEFERRED_PAIR(h[4].expected_active_kind=0);
  REJECT_DEFERRED_PAIR(h[4].expected_active_kind=7);
  REJECT_DEFERRED_PAIR(h[4].flags=HOOK_PREARM_PERMITTED);
  REJECT_DEFERRED_PAIR(h[6].flags=HOOK_PREARM_PERMITTED);
  REJECT_DEFERRED_PAIR(h[4].range_count=1);
  REJECT_DEFERRED_PAIR(h[6].range_count=1);
  REJECT_DEFERRED_PAIR(h[4].reserved[0]=1);
  REJECT_DEFERRED_PAIR(h[6].reserved[0]=1);

  RESTORE_DEFERRED_PAIR();
  c.reset_service_kind=10;
  for (uint32_t i=0;i<6;i++) k[i].kind_id=(uint8_t)((i+1)*10);
  h[0].pc=0x20; h[0].service_kind=60; h[0].opcode_length=1;
  memset(h[0].opcode,0,sizeof(h[0].opcode)); h[0].opcode[0]=0xa0;
  h[1].pc=0x68; h[1].service_kind=20; h[1].expected_active_kind=60; h[1].opcode[0]=0xa1;
  h[2].pc=0xb0; h[2].service_kind=30; h[2].expected_active_kind=60; h[2].opcode[0]=0xa2;
  h[3].pc=0x222; h[3].service_kind=60; h[3].expected_active_kind=40;
  h[3].opcode_length=1;
  memset(h[3].opcode,0,sizeof(h[3].opcode)); h[3].opcode[0]=0xb0;
  h[4].pc=0x300; h[4].service_kind=40; h[4].expected_active_kind=60; h[4].opcode_length=1;
  memset(h[4].opcode,0,sizeof(h[4].opcode)); h[4].opcode[0]=0xb1;
  for (uint32_t i=5;i<=9;i++)
  {
    h[i].pc=0x320; h[i].opcode_length=2; memset(h[i].opcode,0,sizeof(h[i].opcode));
    h[i].opcode[0]=0xb2; h[i].opcode[1]=0xb3;
  }
  h[5].expected_active_kind=20; h[6].expected_active_kind=20; h[6].service_kind=40;
  h[7].expected_active_kind=30; h[8].expected_active_kind=30; h[8].service_kind=40;
  h[9].expected_active_kind=60; h[9].service_kind=40;
  h[10].pc=0x320; h[10].expected_active_kind=60; h[10].opcode_length=2;
  memset(h[10].opcode,0,sizeof(h[10].opcode)); h[10].opcode[0]=0xb2;
  h[10].opcode[1]=0xb3;
  h[11].pc=0x340; h[11].expected_active_kind=40; h[11].opcode_length=1;
  memset(h[11].opcode,0,sizeof(h[11].opcode)); h[11].opcode[0]=0xb4;
  h[12].pc=0x360; h[12].expected_active_kind=40; h[12].opcode_length=1;
  memset(h[12].opcode,0,sizeof(h[12].opcode)); h[12].opcode[0]=0xb5;
  memset(mask,0,sizeof(mask));
  mask[0x20>>3]|=(uint8_t)(1u<<(0x20&7));
  mask[0x68>>3]|=(uint8_t)(1u<<(0x68&7));
  mask[0xb0>>3]|=(uint8_t)(1u<<(0xb0&7));
  mask[0x222>>3]|=(uint8_t)(1u<<(0x222&7));
  assert(gpgx_audio_trace_disable()==TRACE_OK);
  assert(gpgx_audio_trace_configure(&c,mask,k,h,&r)==TRACE_OK);
  memmove(&h[3],&h[2],10*sizeof(h[0])); h[2]=h[1]; h[2].hook_token=13;
  h[2].pc=0x70; h[2].service_kind=50; k[4].flags=KIND_ALLOW_CHILDREN;
  c.hook_count=13; c.snapshot_bytes_total=10;
  mask[0x70>>3]|=(uint8_t)(1u<<(0x70&7));
  assert(gpgx_audio_trace_disable()==TRACE_OK);
  assert(gpgx_audio_trace_configure(&c,mask,k,h,&r)==TRACE_ABI_OR_CONFIG_LIMIT);
#undef REJECT_DEFERRED_PAIR
  RESTORE_DEFERRED_PAIR();
  memmove(&h[12],&h[11],2*sizeof(h[0]));
  memset(&h[11],0,sizeof(h[11]));
  h[11].hook_token=14; h[11].action=ACTION_OBSERVATION_MARKER;
  h[11].cpu=GPGX_AUDIO_TRACE_CPU_M68K; h[11].pc=0x71ba0;
  h[11].expected_active_kind=2; h[11].opcode_length=2;
  h[11].opcode[0]=0x4e; h[11].opcode[1]=0x71;
  rom[(0x71ba0u&0xffffu)^1u]=0x4e;
  rom[((0x71ba0u+1u)&0xffffu)^1u]=0x71;
  c.hook_count=14;
#define START_RESERVED_ROOT() do { \
  assert(gpgx_audio_trace_disable()==TRACE_OK); \
  assert(gpgx_audio_trace_configure(&c,mask,k,h,&r)==TRACE_OK); \
  assert(gpgx_audio_trace_begin_frame()==TRACE_OK); \
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x3a); \
  blocker=trace_stack[0].token; \
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b4c); \
  assert(trace_deferred_begin.pending && trace_stack[0].token==blocker); \
} while(0)
#define START_TRANSFERRED_ROOT(tail_pc, successor_kind) do { \
  START_RESERVED_ROOT(); \
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,(tail_pc)); \
  assert(!trace_runtime_error && trace_deferred_begin.pending && trace_depth==1 \
    && trace_stack[0].kind==(successor_kind)); \
  successor=trace_stack[0].token; \
} while(0)
#define ASSERT_FIRST_FAULT(expected_reason, expected_cpu, expected_pc, expected_kind) do { \
  memset(&fault,0,sizeof(fault)); \
  assert(gpgx_audio_trace_first_fault(&fault)==TRACE_OK); \
  assert(fault.reason==(expected_reason) && fault.source_cpu==(expected_cpu) \
    && fault.pc==(expected_pc) && fault.active_kind==(expected_kind)); \
} while(0)
  ...
  for (uint32_t i=0;i<4;i++)
    gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b4c);
  assert(trace_event_count_value==before+4 && trace_depth==1);
  assert(trace_deferred_begin.pending && TRACE_DEFERRED_ORIGIN(trace_deferred_begin).token==blocker
    && TRACE_DEFERRED_ORIGIN(trace_deferred_begin).kind==6
    && TRACE_DEFERRED_ORIGIN(trace_deferred_begin).parent==0
    && TRACE_DEFERRED_ORIGIN(trace_deferred_begin).depth==0 && trace_deferred_begin.target_kind==4
    && trace_deferred_begin.hook_token==5 && trace_deferred_begin.cpu==GPGX_AUDIO_TRACE_CPU_M68K
    && trace_deferred_begin.pc==0x71b4c);
  assert(trace_events[before].kind==EVENT_HOOK_MARKER && trace_events[before].value==4
    ...
    && trace_events[before].subject==5 && trace_events[before].pc==0x71b4c);
  assert(trace_events[before+3].kind==EVENT_HOOK_MARKER
    && trace_events[before+3].value==4 && trace_events[before+3].service_token==blocker);
  previous=gpgx_audio_trace_enter_cpu(GPGX_AUDIO_TRACE_CPU_Z80);
  gpgx_audio_trace_fm_write(0,0x2a); gpgx_audio_trace_leave_cpu(previous);
  assert(!trace_runtime_error && trace_events[before+4].service_token==blocker
    && trace_events[before+4].source_cpu==GPGX_AUDIO_TRACE_CPU_Z80);
  before=trace_event_count_value;
  ...
  assert(!trace_deferred_begin.pending && trace_depth==2
    ...
    && trace_stack[1].kind==4 && trace_stack[1].parent==blocker
    && trace_stack[1].depth==1);
  child=trace_stack[1].token;
  assert(trace_event_count_value==before+1 && trace_events[before].kind==EVENT_SERVICE_BEGIN
    && trace_events[before].service_token==child && trace_events[before].service_kind==4
    && trace_events[before].parent_token==blocker && trace_events[before].depth==1
    && trace_events[before].subject==10 && trace_events[before].pc==0x71b82
    && trace_events[before].source_cpu==GPGX_AUDIO_TRACE_CPU_M68K);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71bb2);
  previous=gpgx_audio_trace_enter_cpu(GPGX_AUDIO_TRACE_CPU_M68K);
  gpgx_audio_trace_psg_write(0x9f); gpgx_audio_trace_leave_cpu(previous);
  assert(trace_events[before+1].kind==EVENT_HOOK_MARKER
    && trace_events[before+1].service_token==child
    && trace_events[before+2].kind==EVENT_PSG_WRITE
    && trace_events[before+2].service_token==child
    && trace_events[before+2].source_cpu==GPGX_AUDIO_TRACE_CPU_M68K);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71c4c);
  assert(trace_depth==1 && trace_stack[0].token==blocker && trace_stack[0].kind==6
    && trace_events[trace_event_count_value-1].kind==EVENT_SERVICE_END
    && trace_events[trace_event_count_value-1].service_token==child);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  assert(trace_depth==1 && trace_stack[0].kind==2 && trace_stack[0].parent==0
    && trace_stack[0].depth==0 && !trace_runtime_error);
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);

  /* With no reservation, the kind-2 partner at the shared PC stays action 7. */
  assert(gpgx_audio_trace_disable()==TRACE_OK);
  assert(gpgx_audio_trace_configure(&c,mask,k,h,&r)==TRACE_OK);
  assert(gpgx_audio_trace_begin_frame()==TRACE_OK);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x3a);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  successor=trace_stack[0].token; before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(trace_event_count_value==before+1 && !trace_deferred_begin.pending
    && trace_depth==1 && trace_stack[0].token==successor && trace_stack[0].kind==2);
  assert(trace_events[before].kind==EVENT_HOOK_MARKER
    && trace_events[before].service_token==successor
    && trace_events[before].service_kind==2 && trace_events[before].value==3
    && trace_events[before].subject==6 && trace_events[before].pc==0x71b82);
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);

  /* Kind 6 -> kind 2 is one exact five-event transaction, then action 12 consumes. */
  assert(gpgx_audio_trace_disable()==TRACE_OK);
  assert(gpgx_audio_trace_configure(&c,mask,k,h,&r)==TRACE_OK);
  assert(gpgx_audio_trace_begin_frame()==TRACE_OK);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x3a);
  blocker=trace_stack[0].token;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b4c);
  reservation_before=trace_deferred_begin;
  before=trace_event_count_value; next_token=trace_next_token;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  assert(!trace_runtime_error && trace_event_count_value==before+5
    && trace_next_token==(uint16_t)(next_token+1) && trace_depth==1);
  assert(trace_events[before].kind==EVENT_SNAPSHOT_BEGIN
    && trace_events[before+1].kind==EVENT_SNAPSHOT_CHUNK
    && trace_events[before+2].kind==EVENT_SNAPSHOT_END
    && trace_events[before+3].kind==EVENT_SERVICE_END
    && trace_events[before+4].kind==EVENT_SERVICE_BEGIN);
  assert(trace_events[before+3].service_token==blocker
    && trace_events[before+3].service_kind==6 && trace_events[before+3].subject==2
    && trace_events[before+3].pc==0x77 && trace_events[before+3].source_cpu==GPGX_AUDIO_TRACE_CPU_Z80);
  successor=trace_events[before+4].service_token;
  assert(successor==next_token && successor!=blocker
    && trace_events[before+4].service_kind==2 && trace_events[before+4].parent_token==0
    && trace_events[before+4].depth==0 && trace_events[before+4].subject==2
    && trace_events[before+4].pc==0x77 && trace_events[before+4].source_cpu==GPGX_AUDIO_TRACE_CPU_Z80);
  assert(trace_stack[0].token==successor && trace_stack[0].kind==2
    && trace_stack[0].parent==0 && trace_stack[0].depth==0);
  assert(trace_deferred_begin.pending
    && !memcmp(&TRACE_DEFERRED_ORIGIN(trace_deferred_begin),
      &TRACE_DEFERRED_ORIGIN(reservation_before),sizeof(struct trace_stack_entry))
    && TRACE_DEFERRED_ORIGIN(trace_deferred_begin).token==blocker
    && TRACE_DEFERRED_CURRENT_OWNER(trace_deferred_begin).token==successor
    && TRACE_DEFERRED_CURRENT_OWNER(trace_deferred_begin).kind==2);
  previous=gpgx_audio_trace_enter_cpu(GPGX_AUDIO_TRACE_CPU_Z80);
  gpgx_audio_trace_fm_write(0,0x2a); gpgx_audio_trace_leave_cpu(previous);
  assert(trace_events[trace_event_count_value-1].kind==EVENT_FM_WRITE
    && trace_events[trace_event_count_value-1].service_token==successor
    && trace_events[trace_event_count_value-1].service_kind==2
    && trace_events[trace_event_count_value-1].source_cpu==GPGX_AUDIO_TRACE_CPU_Z80);
  before=trace_event_count_value; next_token=trace_next_token;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(!trace_runtime_error && !trace_deferred_begin.pending
    && trace_event_count_value==before+1 && trace_next_token==(uint16_t)(next_token+1)
    && trace_depth==2 && trace_stack[0].token==successor && trace_stack[0].kind==2);
  child=trace_stack[1].token;
  assert(trace_stack[1].kind==4 && trace_stack[1].parent==successor
    && trace_stack[1].depth==1 && trace_events[before].kind==EVENT_SERVICE_BEGIN
    && trace_events[before].service_token==child && trace_events[before].service_kind==4
    && trace_events[before].parent_token==successor && trace_events[before].depth==1
    && trace_events[before].subject==7 && trace_events[before].pc==0x71b82);
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);

  /* The configured kind-3 route is symmetric and needs no route-specific runtime branch. */
  assert(gpgx_audio_trace_disable()==TRACE_OK);
  assert(gpgx_audio_trace_configure(&c,mask,k,h,&r)==TRACE_OK);
  assert(gpgx_audio_trace_begin_frame()==TRACE_OK);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x3a);
  ...
  reservation_before=trace_deferred_begin;
  before=trace_event_count_value; next_token=trace_next_token;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0xc1);
  assert(!trace_runtime_error && trace_event_count_value==before+5
    && trace_next_token==(uint16_t)(next_token+1) && trace_depth==1
    && trace_stack[0].kind==3 && trace_stack[0].parent==0 && trace_stack[0].depth==0);
  assert(trace_events[before].kind==EVENT_SNAPSHOT_BEGIN
    && trace_events[before+1].kind==EVENT_SNAPSHOT_CHUNK
    && trace_events[before+2].kind==EVENT_SNAPSHOT_END
    && trace_events[before+3].kind==EVENT_SERVICE_END
    ...
    && trace_events[before+3].subject==3 && trace_events[before+4].subject==3);
  successor=trace_stack[0].token;
  assert(trace_deferred_begin.pending
    && !memcmp(&TRACE_DEFERRED_ORIGIN(trace_deferred_begin),
      &TRACE_DEFERRED_ORIGIN(reservation_before),sizeof(struct trace_stack_entry))
    && TRACE_DEFERRED_CURRENT_OWNER(trace_deferred_begin).token==successor
    && TRACE_DEFERRED_CURRENT_OWNER(trace_deferred_begin).kind==3);
  before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(!trace_runtime_error && !trace_deferred_begin.pending
    && trace_event_count_value==before+1 && trace_depth==2
    && trace_stack[0].token==successor && trace_stack[0].kind==3
    && trace_stack[1].kind==4 && trace_stack[1].parent==successor
    && trace_events[before].subject==9 && trace_events[before].parent_token==successor);
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);

  /* A pending token/parent/kind/depth mismatch selects neither shared-PC hook. */
#define ASSERT_PENDING_OWNER_MISMATCH(change, fault_kind) do { \
  START_TRANSFERRED_ROOT(0x77,2); \
  change; \
  memcpy(stack_before,trace_stack,sizeof(trace_stack)); \
  reservation_before=trace_deferred_begin; before=trace_event_count_value; \
  next_token=trace_next_token; \
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82); \
  assert(trace_event_count_value==before && trace_next_token==next_token \
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack)) \
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)) \
    && trace_deferred_begin.pending); \
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_SERVICE,GPGX_AUDIO_TRACE_CPU_M68K,0x71b82,(fault_kind)); \
} while(0)
  ASSERT_PENDING_OWNER_MISMATCH(trace_stack[0].token++,2);
  ASSERT_PENDING_OWNER_MISMATCH(trace_stack[0].parent=9,2);
  ASSERT_PENDING_OWNER_MISMATCH(trace_stack[0].kind=3,3);
  ASSERT_PENDING_OWNER_MISMATCH(trace_stack[0].depth=1,2);
#undef ASSERT_PENDING_OWNER_MISMATCH

  /* An unrelated M68K hook cannot replace the pending consume. */
  START_TRANSFERRED_ROOT(0x77,2);
  memcpy(stack_before,trace_stack,sizeof(trace_stack));
  reservation_before=trace_deferred_begin; before=trace_event_count_value;
  next_token=trace_next_token;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71ba0);
  assert(trace_event_count_value==before && trace_next_token==next_token
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_SERVICE,GPGX_AUDIO_TRACE_CPU_M68K,0x71ba0,2);

  /* Five free slots succeed atomically; one-short capacity preserves every state scalar. */
  START_RESERVED_ROOT();
  reservation_before=trace_deferred_begin; next_token=trace_next_token;
  trace_event_count_value=GPGX_AUDIO_TRACE_EVENT_CAPACITY-5; trace_omitted_count=0;
  before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  assert(trace_event_count_value==GPGX_AUDIO_TRACE_EVENT_CAPACITY
    && trace_omitted_count==0 && trace_next_token==(uint16_t)(next_token+1)
    && trace_depth==1 && trace_stack[0].kind==2 && trace_deferred_begin.pending
    && !memcmp(&TRACE_DEFERRED_ORIGIN(trace_deferred_begin),
      &TRACE_DEFERRED_ORIGIN(reservation_before),sizeof(struct trace_stack_entry)));
  assert(trace_events[before].kind==EVENT_SNAPSHOT_BEGIN
    && trace_events[before+1].kind==EVENT_SNAPSHOT_CHUNK
  ...
  before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  assert(trace_event_count_value==before && trace_omitted_count==5
    && trace_next_token==next_token && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before))
    && trace_deferred_begin.pending);

  /* Successor token exhaustion is preflighted without advancing the cursor or ending origin. */
  START_RESERVED_ROOT();
  memcpy(stack_before,trace_stack,sizeof(trace_stack));
  reservation_before=trace_deferred_begin; trace_next_token=0;
  before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  assert(trace_event_count_value==before && trace_omitted_count==0
    && trace_next_token==0 && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_TOKEN,GPGX_AUDIO_TRACE_CPU_Z80,0x77,6);

  /* Wrong reservation target and a non-family tail fail before topology mutation. */
  START_RESERVED_ROOT();
  trace_deferred_begin.target_kind=5;
  memcpy(stack_before,trace_stack,sizeof(trace_stack));
  reservation_before=trace_deferred_begin; next_token=trace_next_token;
  before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  assert(trace_event_count_value==before && trace_next_token==next_token
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_SERVICE,GPGX_AUDIO_TRACE_CPU_Z80,0x77,6);

  /* A late snapshot-source proof failure cannot publish a partial completion. */
  START_RESERVED_ROOT();
  trace_ranges[0].flags=RANGE_M68K_RAM;
  memcpy(stack_before,trace_stack,sizeof(trace_stack));
  reservation_before=trace_deferred_begin; next_token=trace_next_token;
  before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  assert(trace_event_count_value==before && trace_next_token==next_token
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_SNAPSHOT,GPGX_AUDIO_TRACE_CPU_Z80,0x77,6);
  trace_ranges[0].flags=RANGE_Z80_RAM;

  START_RESERVED_ROOT();
  swap=h[1]; swap.hook_token=15;
  memcpy(stack_before,trace_stack,sizeof(trace_stack));
  reservation_before=trace_deferred_begin; next_token=trace_next_token;
  before=trace_event_count_value;
  tail_pop_push(&swap,GPGX_AUDIO_TRACE_CPU_Z80,0x77);
  assert(trace_runtime_error && trace_event_count_value==before && trace_next_token==next_token
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));

  /* A second tail after transfer cannot produce a second owner or token. */
  START_TRANSFERRED_ROOT(0x77,2);
  memcpy(stack_before,trace_stack,sizeof(trace_stack));
  reservation_before=trace_deferred_begin; next_token=trace_next_token;
  before=trace_event_count_value;
  ...
  /* Consume succeeds with one slot and is atomic with zero slots or no token. */
  START_TRANSFERRED_ROOT(0x77,2);
  trace_event_count_value=GPGX_AUDIO_TRACE_EVENT_CAPACITY-1; trace_omitted_count=0;
  before=trace_event_count_value; next_token=trace_next_token;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(trace_event_count_value==GPGX_AUDIO_TRACE_EVENT_CAPACITY
    ...
    && trace_stack[1].parent==successor && trace_stack[1].kind==4
    && trace_events[before].kind==EVENT_SERVICE_BEGIN);
  ...
  before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(trace_event_count_value==before && trace_omitted_count==1
    && trace_next_token==next_token && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ...
  assert(trace_event_count_value==before && trace_omitted_count==0
    && trace_next_token==0 && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_TOKEN,GPGX_AUDIO_TRACE_CPU_M68K,0x71b82,2);

  /* Transfer can cross a frame; only the real current owner ages before consume. */
  START_TRANSFERRED_ROOT(0x77,2);
  reservation_before=trace_deferred_begin;
  assert(gpgx_audio_trace_end_frame()==TRACE_OK);
  assert(gpgx_audio_trace_event_count(&count,&overflow)==TRACE_OK && overflow==0);
  assert(gpgx_audio_trace_drain(drained_events,32,&drained)==TRACE_OK && drained==count);
  assert(gpgx_audio_trace_begin_frame()==TRACE_OK);
  assert(trace_depth==1 && trace_stack[0].token==successor
    && trace_stack[0].kind==2 && trace_stack[0].carried_frames==1
    && trace_deferred_begin.pending
    && !memcmp(&TRACE_DEFERRED_ORIGIN(trace_deferred_begin),
      &TRACE_DEFERRED_ORIGIN(reservation_before),sizeof(struct trace_stack_entry))
    && TRACE_DEFERRED_CURRENT_OWNER(trace_deferred_begin).token==successor);
  ...
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(trace_event_count_value==before+1 && !trace_deferred_begin.pending
    && trace_depth==2 && trace_stack[0].carried_frames==1
    && trace_stack[1].parent==successor && trace_stack[1].carried_frames==0);

  /* Reset/power/abort fail closed after transfer, while disable clears both identities. */
  START_TRANSFERRED_ROOT(0x77,2);
  memcpy(stack_before,trace_stack,sizeof(trace_stack));
  reservation_before=trace_deferred_begin; next_token=trace_next_token;
  before=trace_event_count_value; gpgx_audio_trace_reset_begin(1);
  assert(trace_event_count_value==before && trace_next_token==next_token
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_SERVICE,GPGX_AUDIO_TRACE_CPU_RESET,0,2);
  assert(gpgx_audio_trace_abort_frame()==TRACE_ABI_OR_CONFIG_LIMIT
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ...
  /* The first fault is preserved when later pending violations occur. */
  START_TRANSFERRED_ROOT(0x77,2);
  before=trace_event_count_value;
  previous=gpgx_audio_trace_enter_cpu(GPGX_AUDIO_TRACE_CPU_M68K);
  gpgx_audio_trace_fm_write(0,0x2a); gpgx_audio_trace_leave_cpu(previous);
  assert(trace_event_count_value==before);
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_CHIP_OWNERSHIP,GPGX_AUDIO_TRACE_CPU_M68K,0x71b4c,2);
  preserved_fault=fault;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71ba0);
  assert(gpgx_audio_trace_first_fault(&fault)==TRACE_OK
    && !memcmp(&fault,&preserved_fault,sizeof(fault)));
  ...
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_Z80,0x3a);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b4c);
  rom[(0x71b82u&0xffffu)^1u]=0xff; before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(trace_event_count_value==before && trace_deferred_begin.pending);
  assert(gpgx_audio_trace_end_frame()==TRACE_ABI_OR_CONFIG_LIMIT);
  assert(gpgx_audio_trace_abort_frame()==TRACE_ABI_OR_CONFIG_LIMIT);
  rom[(0x71b82u&0xffffu)^1u]=0x4d;
  ...
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(trace_event_count_value==GPGX_AUDIO_TRACE_EVENT_CAPACITY
    && trace_omitted_count==1 && trace_depth==1 && trace_stack[0].token==blocker
  ...
  assert(trace_event_count_value==before+1 && trace_events[before].kind==EVENT_SERVICE_BEGIN
    && !trace_deferred_begin.pending && trace_depth==2
    && trace_stack[0].token==blocker && trace_stack[0].carried_frames==1
    && trace_stack[1].parent==blocker && trace_stack[1].carried_frames==0);
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);
  ...
  assert(trace_event_count_value==before && trace_depth==1
    && trace_stack[0].token==blocker && !trace_deferred_begin.pending);
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_SERVICE,
    GPGX_AUDIO_TRACE_CPU_M68K,0x71b82,6);
  assert(gpgx_audio_trace_end_frame()==TRACE_ABI_OR_CONFIG_LIMIT);
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);

#define START_NESTED_ORIGIN() do { \
  assert(gpgx_audio_trace_disable()==TRACE_OK); \
  assert(gpgx_audio_trace_configure(&c,mask,k,h,&r)==TRACE_OK); \
  assert(gpgx_audio_trace_begin_frame()==TRACE_OK); \
  memset(trace_stack,0,sizeof(trace_stack)); trace_depth=2; \
  trace_stack[0].token=41; trace_stack[0].kind=4; \
  trace_stack[1].token=42; trace_stack[1].parent=41; \
  trace_stack[1].kind=6; trace_stack[1].depth=1; trace_next_token=43; \
} while(0)
#define ASSERT_NESTED_ORIGIN_MISMATCH(change) do { \
  START_NESTED_ORIGIN(); change; \
  memcpy(stack_before,trace_stack,sizeof(trace_stack)); \
  reservation_before=trace_deferred_begin; before=trace_event_count_value; \
  next_token=trace_next_token; \
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82); \
  assert(trace_event_count_value==before && trace_next_token==next_token \
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack)) \
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before))); \
  ASSERT_FIRST_FAULT(GPGX_AUDIO_TRACE_FAULT_SERVICE,GPGX_AUDIO_TRACE_CPU_M68K,0x71b82,6); \
} while(0)

  /* With no reservation, only the exact configured kind-4 parent -> kind-6
     child topology selects the action-7 half of the driverinput pair. */
  START_NESTED_ORIGIN();
  memcpy(stack_before,trace_stack,sizeof(trace_stack));
  reservation_before=trace_deferred_begin; before=trace_event_count_value;
  next_token=trace_next_token;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(!trace_runtime_error && trace_event_count_value==before+1
    && trace_events[before].kind==EVENT_HOOK_MARKER
    && trace_events[before].value==3 && trace_events[before].subject==13
    && trace_events[before].service_token==42
    && trace_events[before].parent_token==41
    && trace_events[before].service_kind==6 && trace_events[before].depth==1
    && trace_next_token==next_token
    && !memcmp(stack_before,trace_stack,sizeof(trace_stack))
    && !memcmp(&reservation_before,&trace_deferred_begin,sizeof(reservation_before)));
  ...
  ASSERT_NESTED_ORIGIN_MISMATCH(trace_stack[0].kind=2);
  ASSERT_NESTED_ORIGIN_MISMATCH(trace_stack[1].parent=40);
  ASSERT_NESTED_ORIGIN_MISMATCH(trace_depth=3;
    trace_stack[2]=trace_stack[1]; trace_stack[2].token=43;
    trace_stack[2].parent=42; trace_stack[2].depth=2);
#undef ASSERT_NESTED_ORIGIN_MISMATCH
#undef START_NESTED_ORIGIN
  ...
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  assert(trace_event_count_value==before && trace_depth==2
    && trace_stack[1].token==child && !trace_deferred_begin.pending);
  assert(gpgx_audio_trace_end_frame()==TRACE_ABI_OR_CONFIG_LIMIT);
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);
  ...
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b4c);
  before=trace_event_count_value;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b84);
  assert(trace_event_count_value==before && trace_deferred_begin.pending
    && trace_depth==1 && trace_stack[0].token==blocker && !trace_runtime_error);
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71b82);
  child=trace_stack[1].token;
  gpgx_audio_trace_instruction(GPGX_AUDIO_TRACE_CPU_M68K,0x71c4c);
  assert(gpgx_audio_trace_abort_frame()==TRACE_OK);
...
#undef START_RESERVED_ROOT
#undef RESTORE_DEFERRED_PAIR
}
