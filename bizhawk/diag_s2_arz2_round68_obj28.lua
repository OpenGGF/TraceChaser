-- Round 68 ARZ2 Obj3E/Obj28 diagnostic.
-- Copied from diag_template_fast.lua and filled with focused S2 object reads.
...
local function slot_line(label, slot)
    ...
    return string.format(
        "%s s%02X id=%02X rtn=%02X rs=%02X x=%04X.%04X y=%04X.%04X xv=%04X yv=%04X map=%02X rf=%02X sub=%02X o36=%04X o38=%02X",
        label, slot, u8(a), u8(a + 0x24), u8(a + 0x25),
        u16(a + 0x08), u16(a + 0x0A), u16(a + 0x0C), u16(a + 0x0E),
        u16(a + 0x10), u16(a + 0x12), u8(a + 0x1A), u8(a + 0x01),
        u8(a + 0x28), u16(a + 0x36), u8(a + 0x38))
end
...
local function context(label)
    ...
    local a0slot = math.floor(((a0 & 0xFFFF) - 0xB000) / SLOT_SIZE)
    local a1slot = math.floor(((a1 & 0xFFFF) - 0xB000) / SLOT_SIZE)
    log(string.format(
        "bk2=%d trace=%d vfc=%04X vint=%04X pc=%06X %-16s a0=%04X/s%02X a1=%04X/s%02X d0=%04X d1=%04X rng=%04X%04X",
        f, trace, u16(ADDR_VFC), u16(ADDR_VINT_LOW), pc, label,
        a0 & 0xFFFF, a0slot, a1 & 0xFFFF, a1slot, d0 & 0xFFFF, d1 & 0xFFFF,
        u16(ADDR_RNG_HI), u16(ADDR_RNG_LO)))
    if a0slot >= 16 and a0slot <= 45 then log("  " .. slot_line("a0", a0slot)) end
    if a1slot >= 16 and a1slot <= 45 then log("  " .. slot_line("a1", a1slot)) end
    log("  " .. slot_line("slot1C", 0x1C))
end

event.onmemoryexecute(function() context("Obj28_Main") end, 0x011ADE, "arz2_obj28_main")
event.onmemoryexecute(function() context("Obj28_Walk") end, 0x011B38, "arz2_obj28_walk")
event.onmemoryexecute(function() context("Obj28_Fly") end, 0x011B74, "arz2_obj28_fly")
event.onmemoryexecute(function() context("Obj28_Prison") end, 0x011BF4, "arz2_obj28_prison")
event.onmemoryexecute(function() context("Obj3E_Random") end, 0x03F3A8, "arz2_obj3e_random")
event.onmemoryexecute(function() context("AllocateObject") end, 0x017FDA, "arz2_allocate")
event.onmemoryexecute(function() context("DeleteObject") end, 0x0164E4, "arz2_delete")
...
local function sample()
    ...
    if f < START or f > STOP then return end
    local trace = f - TRACE_OFFSET
    local line = string.format("bk2=%d trace=%d vfc=%04X vint=%04X rng=%04X%04X | %s | %s | %s | %s | %s | %s",
        f, trace, u16(ADDR_VFC), u16(ADDR_VINT_LOW), u16(ADDR_RNG_HI), u16(ADDR_RNG_LO),
        slot_line("s10", 0x10), slot_line("s1C", 0x1C), slot_line("s1D", 0x1D),
        slot_line("s20", 0x20), slot_line("s21", 0x21), slot_line("s22", 0x22))
    if line ~= last then
        log(line)
        last = line
    end
