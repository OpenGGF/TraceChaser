-- Does RunObject's loop REACH Obj05's slot on rows 5554-5564, and what id does
-- it read there?
--
...
-- RunObject (docs/s2disasm/s2.asm:29832, sub_15FCC) is the loop body. Hooking it
-- gated to a0 = Obj05's slot fires when the dispatch loop REACHES that slot,
-- whether or not the `move.b id(a0),d0 / beq RunNextObject` skip then fires. So
-- "no hit" means the loop never got there; "hit with id=00" means it got there
-- and skipped an emptied slot. Those are the two remaining candidates and the
-- Obj05-side probe cannot tell them apart.
ProbeRuntime.run({
    stage = function()
    ...
    hooks = {
        ...
        { name = "RunObject", address = 0x015FCC,
          callback = onObj05Site("RUNOBJECT_REACHED") },
