...
no-audio diagnostic config, and invokes EmuHawk with normal Windows quoting.
The Lua template also mutes audio with `client.SetSoundOn(false)` and disables
rendering with `client.invisibleemulation(true)`. This avoids BizHawk 2.11 failures such as
