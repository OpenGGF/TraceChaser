...
-- BizHawk Lua script for recording a Sonic 3 & Knuckles COMPLETE-RUN movie
-- (AIZ -> Doomsday) into per-ZONE physics-trace segments during a single
-- BK2 movie playback pass.
--
-- Derived from s3k_trace_recorder.lua. The full S3K per-frame CSV schema and
-- every aux event (cpu_state, oscillation_state, object_state, interact_state,
-- velocity/position writes, control_lock, etc.) are kept verbatim so each
-- segment is byte-compatible with the existing single-arm S3K fixtures and
-- the AbstractTraceReplayTest pipeline.
--
-- Key behavioural difference from the single-arm recorder:
--   * Instead of arming once and finalising when gameplay is left, this
--     recorder auto-segments PER ZONE. A NEW segment begins when a fresh
--     level entry for a *different* zone occurs:
--         is_level_family_mode(game_mode) AND zone != current_segment_zone.
--   * It does NOT re-arm on act change -- the seamless act1->act2 transition
--     stays inside one segment.
--   * The trailing 0x8C zone-exit handoff frames (zone register already moved
--     to the next zone, game_mode 0x8C) are recorded into the CURRENT segment
--     until the next zone's 0x0C arms the next segment, giving each segment
--     "through the zone-exit handoff".
--   * Each segment writes to OUTPUT_DIR = BASE/<zoneToken>/ with its own
--     physics.csv + aux_state.jsonl + metadata.json.
--   * A per-frame Game_paused diagnostic aux flag (0xFFF63A) makes the
--     accidental HCZ pause window visible. Recording is NEVER altered during
--     a pause: frozen frames are recorded verbatim (comparison-only).
--
-- Usage:
--   EmuHawk.exe --chromeless --lua=s3k_complete_run_recorder.lua \
--       --movie=s3k-complete-sonic-tails.bk2 s3k.gen
--   Set OGGF_TRACE_OUTPUT_DIR to an absolute scratch BASE dir.
--
...
-- recorder v3.6.
--
-- v6.28-s3k-completerun: adds the comparison-only aiz_fire_transition_per_frame
-- aux event (V628_AIZ_FIRE) for the AIZ end-to-end profile only. State-poll,
-- no execution hooks: per frame in the ~5200-5600 window it logs
-- Camera_Y_pos_BG_copy (0xEE90 long, the continuous fake-fire BG ramp),
-- Camera_Y_pos_BG_rounded (0xEE96), Events_bg+$00/$02 (lerp target / FireRise
-- speed accumulator, 0xEED2/0xEED4), Events_routine_bg (0xEEC2 BG phase),
-- Events_fg_5 (0xEEC6 fire-start trigger), and Camera_X/min_X/max_X
...
-- Load only the shared modules installed beside this recorder. Caller-selected
...
-- BASE output dir (env). Each per-zone segment writes to BASE/<zoneToken>/.
-- OUTPUT_DIR is repointed per segment in start_new_segment(); the rest of the
-- recorder (open_files/write_metadata/reset_recording_state) consumes
-- OUTPUT_DIR unchanged.
-- NOTE: BASE_OUTPUT_DIR and OUTPUT_DIR are intentionally GLOBALS (no `local`).
-- The base recorder's main chunk is already near Lua's 200-local-per-function
...
-- This recorder is per-zone-auto-segmenting; force a distinct profile so the
-- single-arm aiz_end_to_end / level_gated_reset_aware paths never engage.
...
-- post-reload Camera_max_X_pos clamp (sonic3k.constants.asm:423 ds.l 1 ->
-- $FFFFEE90; s3.asm:70383 AIZ1_FireRise; sonic3k.asm:105084-105096).
-- NOTE: intentionally global (not local) to stay under Lua's 200-local
-- main-chunk limit (the v6.27 additions pushed the main chunk over 200).
ADDR_CAMERA_Y_BG_COPY = 0xEE90
-- Camera_Y_pos_BG_rounded (word) follows the BG copy snapped to a block boundary
-- (sonic3k.constants.asm:425 -> $FFFFEE96).
...
-- $FE11, then Life_count; see the CrossResetRAM walk documented above).
-- Recorded per-frame so a death or 1UP is reportable on the exact frame it
-- happens; comparison-only, never applied to engine state.
...
-- (docs/skdisasm/s3.asm:1694 / sonic3k Pause_Loop). Non-zero while the
-- in-game pause is active. Recorded per-frame as a diagnostic aux flag so the
-- accidental HCZ pause window is visible; recording is NEVER altered during a
-- pause -- frozen frames are written verbatim (comparison-only).
-- GLOBAL (no `local`): keeps the main chunk under Lua's 200-local limit.
ADDR_GAME_PAUSED            = 0xF63A
...
-- Per-zone directory token map (ROM zone id -> dir token). Distinct from
-- ZONE_NAMES so the late-game special slots get unambiguous tokens:
--   22 -> hpz22, 10 -> hpz, 23 -> dez23. Used to set each segment's
-- OUTPUT_DIR = BASE/<zoneToken>/ and the metadata `zone` field.
-- GLOBAL (no `local`): keeps the main chunk under Lua's 200-local limit.
ZONE_TOKEN = {
    [0]  = "aiz",
    [1]  = "hcz",
...
-- GLOBALS (no `local`): keeps the main chunk under Lua's 200-local limit
-- after the per-zone segmentation additions.
...
-- Per-zone segmentation state. current_segment_zone is the ROM zone id the
-- currently-armed segment belongs to (nil before the first segment). A new
-- segment is armed when a fresh level entry occurs for a DIFFERENT zone.
-- segments_done collects {token, zone_id, act, bk2_frame_offset, rows} for
...
local function reset_recording_state(keep_files)
...
local function write_metadata()
    ...
    if TRACE_PROFILE == "complete_run" then
        fixture_notes = "Per-zone segment from the S3K complete-run "
            .. "(AIZ->Doomsday) Sonic+Tails movie. Covers act1 -> seamless "
            .. "act1->act2 -> the act2->next-zone exit handoff (trailing 0x8C "
            .. "frames). Game_paused aux flag is comparison-only."
    elseif is_aiz_end_to_end_profile() then
    ...
    if TRACE_PROFILE == "complete_run" then
        -- The complete-run recorder emits every base per-frame aux event plus
        -- the (window/zone-gated) CNZ/cage/collision diagnostics every frame
        -- they apply. Declare them so segment consumers know they may appear.
...
-- Per-frame Game_paused diagnostic flag. Records the ROM Game_paused byte
-- ($FFFFF63A) every recorded frame so the accidental HCZ in-game pause window
-- is visible to diagnostics. Comparison-only: recording is never altered while
-- paused -- frozen frames are written verbatim like any other frame.
local function write_game_paused_per_frame()
    if not aux_file then return end
    -- Game_paused is a WORD (ds.w 1, sonic3k.constants.asm:564); ROM writes
    -- move.w #1,(Game_paused).w (sonic3k.asm:1545). Read the full word so the
    -- nonzero pause state is visible (reading only the high byte at 0xF63A
    -- would always be 0 for the 0x0001 paused value).
    write_aux(string.format(
        '{"frame":%d,"vfc":%d,"event":"game_paused_state","game_paused":%d}',
        trace_frame,
        mainmemory.read_u16_be(ADDR_FRAMECOUNT),
        mainmemory.read_u16_be(ADDR_GAME_PAUSED)))
end
...
-- =====================================================================
-- AIZ1->AIZ2 fake-fire transition BG-copy ramp (v6.28-s3k)
-- =====================================================================
-- Comparison-only per-frame snapshot of the ROM's single continuous
-- Camera_Y_pos_BG_copy fire-rise ramp, the FireRise speed accumulator, the
-- BG event routine/phase, the Events_fg_5 fire-start trigger, and the camera
-- X clamp. This pins, for the engine's continuous-ramp AIZ fire-transition
-- rewrite: (1) the exact frame Events_fg_5 sets the fire transition going,
-- (2) the Camera_Y_pos_BG_copy value at the AIZ1BGE_Finish reload frame, and
-- (3) the frame the ramp crosses $310 to release Camera_max_X_pos=$6000.
-- ROM refs: s3.asm:70383 AIZ1_FireRise (Events_bg+$02 += $280 cap $A000;
-- Camera_Y_pos_BG_copy += speed<<4); sonic3k.asm:104638 AIZ1_AIZ2_Transition
-- (BG copy init $200000, lerp target Events_bg+$00=$68); :105084-105096
-- AIZ2BGE_WaitFire $310 release. The fields are diagnostic report context
-- only and must never feed replay/engine state.
local V628_AIZ_FIRE = {
    FRAME_START = 5200,
    FRAME_END = 5600,
    enabled = false,
...
function V628_AIZ_FIRE.is_aiz()
    -- AIZ zone id is 0; act 0 (AIZ1) or act 1 (AIZ2) both relevant since the
    -- fake-fire ramp spans the seamless AIZ1->AIZ2 reload.
...
function V628_AIZ_FIRE.write()
    if not aux_file then return end
    if not started then return end
    -- Only enabled for the AIZ end-to-end profile so it does not bloat other
    -- zones' traces; the window further restricts it to the transition region.
...
-- ---------------------------------------------------------------------------
-- Per-zone segmentation helpers
-- ---------------------------------------------------------------------------

-- Finalise the currently-armed segment (if any): write its metadata, close its
-- files, record it in segments_done for the end-of-run report, then reset all
-- per-segment recording state so the next segment starts fresh.
function finalize_segment()
    ...
    local token = zone_token_for(start_zone_id)
    print(string.format(
        "Finalised segment %s (zone_id=%d act=%d): %d rows, bk2_frame_offset=%d.",
        token, start_zone_id, start_act + 1, rows, bk2_frame_offset))
    close_files()
...
-- Arm a new per-zone segment for the given ROM zone id. Repoints OUTPUT_DIR to
-- BASE/<zoneToken>/, captures the segment's start state, opens its files, and
...
function start_new_segment(zone_id)
    ...
    write_metadata()
    print(string.format(
        "Segment armed at BizHawk frame %d, zone %s (id=%d) act %d, pos (%04X, %04X)",
...
function on_frame_end()
    ...
    -- HARD movie-input-end guard (applies whether or not a segment is armed).
    -- The complete-run movie is 466334 input frames; after that BizHawk enters
    -- a "finished" mode that keeps emulating with no input, and the S3K attract
    -- loop carries the game through the ending/credits back to the title and
    -- into a demo level. Without a hard stop the final ddz segment kept
    -- recording thousands of post-movie credits/title/attract-demo frames (and
    -- the per-zone arming would even re-arm a bogus AIZ demo segment). Stop the
    -- instant the BizHawk frame counter reaches the movie's input-frame count.
    -- emu.framecount() is the absolute BizHawk frame and is robust regardless
    -- of movie.mode() reporting quirks in FINISHED state.
    if movie.isloaded() then
        local mlen = movie.length()
        if mlen and mlen > 0 and emu.framecount() >= mlen then
            print(string.format(
                "Reached movie input end (emu frame %d >= movie length %d). Finalising.",
                emu.framecount(), mlen))
            finished = true
            return
        end
    ...
    -- should_start_recording (s3k_trace_recorder.lua):
    --   game_mode == GAMEMODE_LEVEL (raw 0x0C, NOT the 0x4C/0x8C load-
    --   transition family) AND zone != current_segment_zone
    --   AND ctrl_lock_timer == 0 AND ctrl_locked == 0
    -- This is exactly the frame the engine's trace-replay bootstrap is
    -- calibrated to start from. We require RAW 0x0C (not is_level_family_mode)
    -- on purpose: during the 0x4C/0x8C level-load transition the player object
    -- is not placed yet (x_pos/y_pos read 0) and ctrl_locked can briefly read
    -- 0, which would arm at an unsettled frame. Requiring 0x0C means the level
    -- is fully assembled before we test the lock. Arming at the RAW zone-entry
    -- frame (during the locked 0x0C intro, camera still settling under load-
    -- lag) produced a small frame-0 camera/position + lock mismatch in every
    -- segment; gating on control unlock skips only that brief locked intro.
    --
    -- This is a ONE-TIME arm gate per segment. Once a segment is armed it
    -- records CONTINUOUSLY through everything after -- later control-locks,
    -- the seamless act1->act2 transition, and the act2->next-zone exit
    -- handoff -- because the `zone_id ~= current_segment_zone` test only
    -- becomes true again once the NEXT zone register is loaded, and we only
    -- finalize/re-arm there once THAT zone reaches its own raw-0x0C,
    -- control-unlocked frame. The trailing handoff frames (next zone's load
    -- transition + its locked 0x0C intro) are therefore recorded into the
    -- CURRENT segment via the fall-through below ("through the zone-exit
    -- handoff").
    if game_mode == GAMEMODE_LEVEL and zone_id ~= current_segment_zone then
        local ctrl_lock_timer = mainmemory.read_u16_be(PLAYER_BASE + OFF_CTRL_LOCK)
        local ctrl_locked = mainmemory.read_u8(ADDR_CTRL1_LOCKED)
        if ctrl_lock_timer == 0 and ctrl_locked == 0 then
            if started then
            ...
            -- Arm-and-RETURN (do NOT record the arm frame), matching the
            -- dedicated recorder's should_start_recording path. The dedicated
            -- single-arm recorder records the NEXT BizHawk frame as trace
            -- frame 0; the engine's trace-replay bootstrap is calibrated to
            -- that frame. Falling through here recorded the arm frame itself
            -- as row 0 -- one frame too early -- so the recorded frame-0 was
            -- the pre-settle frame (status_byte still 0x00, player not yet
            -- moving) instead of the settled frame the dedicated trace uses.
            -- That off-by-one was the systemic frame-0 camera/lock mismatch.
            -- bk2_frame_offset stays at the arm frame F; trace row 0 is frame
            -- F+1, and the replay convention (row N = bk2 frame
            -- bk2_frame_offset + N) is preserved exactly as in the dedicated
            -- recorder.
            return
        end
    end
    ...
    if not started then
        -- Pre-first-segment frames (Sega/title/level-load 0x4C/0x8C with
        -- zone still 0 before AIZ's 0x0C) -- nothing to record yet.
        return
    end
    ...
    -- Per-frame AIZ1->AIZ2 fake-fire BG-copy ramp snapshot (v6.28 schema).
    -- State-poll only (no execution hooks): Camera_Y_pos_BG_copy, the FireRise
    -- speed accumulator, the BG event routine/phase, Events_fg_5, and the
    -- camera X clamp through the AIZ1->AIZ2 fire transition. Comparison-only.
    V628_AIZ_FIRE.write()
    ...
    -- Per-frame Game_paused diagnostic flag. Makes the accidental HCZ pause
    -- window visible. Comparison-only; frozen frames recorded verbatim.
    write_game_paused_per_frame()
